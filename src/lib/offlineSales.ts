...
export type PendingLine = { product_id: string; quantite: number; prix_unitaire: number; nom: string };
  ...
  for (const s of getPending()) {
    try {
      ...
      const { error: e2 } = await supabase.from("sale_items").insert(
        s.lines.map((l) => ({ sale_id: sale.id, product_id: l.product_id, quantite: l.quantite, prix_unitaire: l.prix_unitaire }))
      );
      if (e2) throw e2;

...
export default function Dashboard() {
  ...
  // Détail d'un impayé : ta marge · coût déjà couvert (marchandise filtropro/capital OU bon
  // marqué « dinoun payé ») · coût encore à payer à dinoun.
  const [impayeDetail, setImpayeDetail] = useState<Record<string, { benefice: number; coutCouvert: number; coutAPayer: number }>>({});
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const detail: Record<string, { benefice: number; coutCouvert: number; coutAPayer: number }> = {};
        for (const s of perSale.values()) {
          const ratio = s.total > 0 ? Math.max(0, s.total - s.paye) / s.total : 1; // part encore due
          const benefice = Math.max(0, s.revenue - (s.coutCouvert + s.coutAPayer));
          const d = detail[s.client_id] ?? { benefice: 0, coutCouvert: 0, coutAPayer: 0 };
          d.benefice += benefice * ratio;
          d.coutCouvert += s.coutCouvert * ratio;
          d.coutAPayer += s.coutAPayer * ratio;
            ...
            {impayes.map(c => {
              ...
              <div key={c.id} className="flex items-center justify-between gap-2 text-sm bg-[var(--surface-2)] rounded-lg px-3 py-2">
                <div className="min-w-0 flex-1">
                  ...
                  {d && (d.benefice + d.coutCouvert + d.coutAPayer) > 0 && (
                    <p className="text-[11px] text-slate-500 mt-0.5">
                      ta marge <span className="text-emerald-400">{d.benefice.toFixed(0)}</span>
                      {d.coutCouvert > 0 && <> · coût couvert <span className="text-emerald-400">{d.coutCouvert.toFixed(0)}</span></>}
                      {d.coutAPayer > 0 && <> · à payer dinoun <span className="text-orange-400">{d.coutAPayer.toFixed(0)}</span></>}
                    </p>
                  )}
                </div>
